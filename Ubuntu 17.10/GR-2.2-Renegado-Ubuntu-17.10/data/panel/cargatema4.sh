...
if [ $playlist -ge 3 ]; then
        sed -i "3 a $tema" ~/.gradio/data/tmp/playlist
        sed -i "3 a $nombrebase "[""$duracionview""]"" ~/.gradio/data/tmp/playlistg3
        sed -i "5d" ~/.gradio/data/tmp/playlist
        sed -i "5d" ~/.gradio/data/tmp/playlistg3
        sed -i "3 a <font size='1' color='green'><b>`echo $tema|awk -F/ '{print $NF}'`</b></font></br>" ~/.gradio/data/tmp/playlistweb
        sed -i "5d" ~/.gradio/data/tmp/playlistweb
        echo $nombrebase "[""$duracionview""]" > ~/.gradio/data/tmp/cartucho4
else
